...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (data.success) {
        setDashboardData(data.dashboardData); // <- use dashboardData
      } else {
      ...
      {dashboardData.recentBlogs.length > 0 && (
        <div className="mt-10">
          <div className="flex items-center gap-3 mb-6">
            <img src={assets.dashboard_icon_4} alt="Recent Blogs" className="w-8 h-8 object-contain" />
            ...
            <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
              ...
              <tbody>
                {dashboardData.recentBlogs.map((blog, index) => (
                  <BlogTableItem key={blog._id} blog={blog} fetchBlog={fetchData} index={index} />
                ))}
              </tbody>
            </table>
          </div>
