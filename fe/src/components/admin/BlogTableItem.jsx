...
const BlogTableItem = ({ blog, index, fetchBlog }) => {
    ...
    return (
        <tr className="border-b hover:bg-gray-50 transition-colors duration-200">
            <th className="py-4 px-6 text-gray-700">{index + 1}</th>
            <td className="py-4 px-6 text-gray-700">{title}</td>
            <td className="py-4 px-6 text-gray-700">{BlogDate.toDateString()}</td>
            <td className="py-4 px-6">
                <span className={`px-2 py-1 rounded-full text-sm ${isPublished ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                    }`}>
            ...
            <td className="py-4 px-6 flex items-center gap-2">
                ...
                <div
                    className="w-6 h-6 flex items-center justify-center bg-red-400 hover:bg-red-500 rounded-full cursor-pointer transition"
                >
